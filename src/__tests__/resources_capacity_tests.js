...
describe('Resources Capacity', () => {
  ...
  it('Player capacity can be multiplied by buildings', () => {
    ...
    player.city.planBuilding({ building: multiplyingBuilding });
  ...
  it('can only earn up to a limit', () => {
    ...
    assert.strictEqual(
      amount * 2,
      player.getResourceAmountForType(resourceType1)
    );
    assert.strictEqual(
      amount * 3,
      player.getResourceAmountForType(resourceType2)
    );
    ...
    assert.strictEqual(
      amount * 1,
      player.getResourceAmountForType(resourceType3)
    );
  });
});
