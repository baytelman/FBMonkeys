...
import {ResourceIcon} from '../resources/ResourceIconComponent';
import GameController  from '../../../lib/controller/GameController';

const MAX_TASKS = 2;

class CharacterDisplay extends Component {
  assignTaskMenu(characterId, availableTasks, assignedTask, taskIndex, assignTaskCallback) {
  ...
  activeTasks(character) {
    let availableTasks = Object
      .values(GameController.instance.module.availableTasks())
      .filter(t => t.isAvailable(this.props.player));
    availableTasks.unshift({namespace: "", name: 'Unassigned'});
