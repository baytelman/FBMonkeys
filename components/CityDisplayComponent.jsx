...
var CityDisplay = React.createClass({
    render: function() {
        let buildings = Object.values(this.props.player.city.buildings).map((b, index) => {
            ...
            <div key={b.id + "_icon"} style={{
                ...
                height: kAssetWidth,
                zIndex: 1,
                backgroundImage: 'url(/images/buildings/' + b.asset + '.png)',
                backgroundRepeat: 'no-repeat',
                backgroundPosition: 'center',
                backgroundSize: 'contain'
            }}>
            <div key={b.id + "_info"} style={{ position: 'absolute', bottom: 0, width: '100%'}}>
                <div key={b.id + "_name"} style={{ 'marginRight': 'auto', 'marginLeft': 'auto'}}>{ b.name }</div>
                <div key={b.id + "_status"} style={{ 'marginRight': 'auto', 'marginLeft': 'auto'}}>{ b.getStatus() }</div>
            </div>
            </div>
            </div>
        });
