...
export class CollectBuildingResourcesEffect extends FrequencyEffect {
  ...
  began(parents) {
    ...
    this.collectingCharacter = parents.character;
    if (!this.building.getStoredResources()) {
      throw new InsuficientResourcesError();
    }
    console.log({character: parents.character.name, building: this.building.id});
    return [];
  }
  trigger(parents) {
    console.log({task_done: this.id, character: parents.character.name, building: this.building.id});
    const event = this.building.collectResources(parents.player);
    this.building = null;
    return [event];
  }
  getDescription() {
