...
export class CityPlayer {
  ...
  canEarnAnyResources(resources) {
    ...
    const canEarn = resources.map(r => capacity[r.type] && capacity[r.type] > ((this.resources[r.type] && this.resources[r.type].amount) || 0)).some(can => can);
    return canEarn;
  }
...
export class BuildingStoreResourceEffect extends FrequencyEffect {
  ...
  canBegin(parents) {
    return (parents.building.getStoredResources() === false) || (parents.player.canEarnAnyResources(this.resources));
  }
  trigger(parents) {
